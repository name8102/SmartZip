# File-Aware CLI Commands (detect / list / test)

## Goal

承接 `07-02-file-grain-history`（数据库文件级模型 + extract 闭合），把 `detect` / `list` / `test` 三个命令接线到文件级历史与 `known_files` 索引，并把三命令共用的**求密码**与**按编码列条目**抽成引擎层共享子流程。
...
- **test**（独立命令，全量校验）：后端 `t` 全量读校验、定位损坏卷，坏卷列表写 `file_extractions.damaged_volumes_json`（报全部坏卷，不只第一个）。status=`intact` / `corrupt`。加密档需密码：走共享求密码流程；试出的密码写 `known_files.password_id`。**extract 不含 test 步骤**（test 是独立重活，全量扫描耗资源，extract 直接 `7z e` 试密码解压，不做校验）。

### 共享子流程（抽到引擎层，三命令共用，避免各写一份）
1. **求密码流程**（list / test / extract 共用）：候选顺序 = 命令行 `--password` → `known_files.password_id`（hash+size 命中，置顶不独占）→ 配置文件通配符（**将来 TODO**）→ `passwords` 库排序。候选试穿 → 交互询问用户输入。成功后：bump `passwords` 计数 + 写 `known_files.password_id`。用户取消/放弃 → status=`skipped` + reason=`password_required`（统一，不为加密单设终态）。
...
- test 加密档：走共享求密码流程；试不出且用户取消 → `skipped` + `password_required`。
...
- **test**：试出密码写 password_id；不写 confirmed_encoding、不写 last_extract_at。
...
- GUI 编码确认交互 / TUI。
- 去重跳过逻辑（属 extract，已在上一任务闭合；detect/list/test 只读不跳过）。
...
- `test <inputs>`：全量校验；损坏档报 `corrupt` 且 `damaged_volumes_json` 列出全部坏卷；完好报 `intact`；加密档经求密码流程校验，试出密码写 known_files.password_id。
- 共享求密码流程被 list / test / extract 三处调用（extract 复用已存在的路径），无重复实现；求密码候选顺序正确，用户取消统一落 `skipped`+`password_required`。
