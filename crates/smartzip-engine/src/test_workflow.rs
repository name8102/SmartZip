            ...
            && !report.unreadable_volumes.contains(&report.entrypoint)
        {
            ...
            loop {
                ...
                let result = tokio::select! { biased;
                    ...
                    result=backend_call("archive-executor","testing",&report.entrypoint,backend.test_with_context(test_request,&context))=>result,
                };
                ...
                && !report.volumes.ambiguous
            {
                if let Some(previous) = previous {
                    ...
                    let result = tokio::select! { biased;
                        ...
                        result=backend.diagnose_test_with_context(test_request,&previous,report.volumes.family!=VolumeFamily::Single,&context)=>result,
                    };
                    match result {
                        Ok(Some(result))=>{
